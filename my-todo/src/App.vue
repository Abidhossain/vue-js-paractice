...
<template>
  <div class="max-w-md mx-auto bg-white shadow-lg rounded-lg overflow-hidden mt-16 border p-6 space-y-6">
    <h1 class="text-2xl uppercase font-bold text-gray-700">To Do List <span v-if="todos.length">({{ countCompletedTodo }} /{{ countTotalTodo }})</span></h1>
      ...
      <li v-for="(todo,index) in todos" :key="todo.id" v-if="todos.length">
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            <input @input="handleCheckboxComplete(todo)" class="h-4 w-4 text-600" type="checkbox" :id="todo.id"
                   :checked="todo.completed">
            <label :for="todo.id"><span class="text-lg font-medium pl-3" :class="todo.completed ? 'line-through' : ''">{{
                todo.title
              }}</span></label>
          </div>
          <button class="flex-shrink-0" @click="removeTodoItem(index)">
